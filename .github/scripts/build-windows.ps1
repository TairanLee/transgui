...
$makeLazarusDir = 'LAZARUS_DIR=' + ($LazarusDir -replace '\\', '/')
$makeVersion = "PROG_VER=$version"
...
foreach ($requiredFile in @($lazbuild, $make, $iscc, $zip, $upx)) {
...
try {
  Invoke-ExternalCommand $lazbuild @(
    '-B',
    (Join-Path $repositoryRoot 'trcomp.lpk'),
    "--lazarusdir=$LazarusDir",
    "--pcp=$primaryConfigPath"
  )

  Invoke-ExternalCommand $lazbuild @(
    '-B',
    (Join-Path $repositoryRoot 'transgui.lpi'),
    "--lazarusdir=$LazarusDir",
    "--pcp=$primaryConfigPath"
  )

  Invoke-ExternalCommand $make @($makeVersion, $makeLazarusDir, 'clean')
  Invoke-ExternalCommand $make @($makeVersion, $makeLazarusDir, 'all')
  ...
  $setupScript = Join-Path $repositoryRoot "$setupDirectory\setup.iss"
  Invoke-ExternalCommand $iscc @(
    '/Qp',
    "/DAppVersion=$version",
    $setupScript
  )
  ...
  Invoke-ExternalCommand $upx @('--best', $executable)
  Invoke-ExternalCommand $upx @('-t', $executable)
  Assert-PeMachine -Path $executable -ExpectedMachine $expectedMachine
  Assert-FileVersion -Path $executable -ExpectedVersion $version
  Assert-Unsigned -Path $executable

  Invoke-ExternalCommand $make @($makeVersion, $makeLazarusDir, 'zipdist')
